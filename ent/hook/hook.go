...
// The TopologyLinkFunc type is an adapter to allow the use of ordinary
// function as TopologyLink mutator.
type TopologyLinkFunc func(context.Context, *ent.TopologyLinkMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f TopologyLinkFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.TopologyLinkMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.TopologyLinkMutation", m)
}
