...
2. **Auto Setup**: Run the integrated setup script to install Python, Pip, and all dependencies automatically:
   ```bash
   chmod +x scripts/setup_env.sh
   ./scripts/setup_env.sh
   ```
3. **Manual Setup (Alternative)**: See [dependencies.md](references/dependencies.md).
...
2. **自动环境搭建**：运行内置脚本自动安装 Python、Pip 及所有系统依赖：
   ```bash
   chmod +x scripts/setup_env.sh
   ./scripts/setup_env.sh
   ```
3. **手动安装（可选）**：参见 [dependencies.md](references/dependencies.md)。
